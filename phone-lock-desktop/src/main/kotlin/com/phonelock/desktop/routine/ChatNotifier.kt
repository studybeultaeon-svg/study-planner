...
/**
 * 채팅 알림 신규(2026-09-10, 사용자 요청 — "진동 없이") — 내가 속한 모임 대화방 + 1:1 DM을 돌면서
 * 마지막으로 확인한 시각 이후 온 메시지가 있으면 트레이 알림([DesktopNotifier])으로 알린다.
 * [SocialGroupNotifier]와 동일한 tick() 구조로 Main.kt의 7초 주기 루프에 얹는다. [ActiveChatTracker]에
 * 지금 보고 있는 방이면 건너뛴다(화면이 이미 폴링 중이라 중복 알림이 된다). 대화방 개수만큼 요청이
 * 늘어나는 구조라 무전기/넛지처럼 가벼운 "최신 메시지 1개만" 조회로 비용을 줄였다(안드로이드판과 대칭).
        ...
        Thread {
            try {
                ...
                groupIds.forEach groupLoop@{ groupId ->
                    if (groupId == ActiveChatTracker.openChatId) return@groupLoop
                    val latest = ChatSyncClient.peekLatestGroupMessage(url, key, groupId) ?: return@groupLoop
                    if (latest.senderUid == myUid) return@groupLoop
                    val lastSeen = repository.chatLastSeenFor(groupId)
                    if (latest.sentAtMillis > lastSeen) {
                        DesktopNotifier.notify("${latest.senderName}님 (모임)", latest.text)
                        repository.setChatLastSeen(groupId, latest.sentAtMillis)
                    }
                }
