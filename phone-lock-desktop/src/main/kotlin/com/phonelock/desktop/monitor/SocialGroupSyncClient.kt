...
object SocialGroupSyncClient {
    ...
    /** 모임장 소유권 승계(77차 IDEAS.md, 92차 소셜 개편 Phase 4, 안드로이드판과 대칭) — 옛 모임장은
     *  권한을 완전히 잃지 않도록 자동으로 관리자가 된다. `groups/{id}/info`는 이미 "모임장 또는 관리자가
     *  쓸 수 있다"는 규칙이라 별도 보안 규칙 추가 없이 동작한다. */
    fun transferOwnership(databaseUrl: String?, apiKey: String?, groupId: String, newOwnerUid: String): Result<Unit> {
        if (databaseUrl.isNullOrBlank() || apiKey.isNullOrBlank()) return Result.failure(IllegalStateException("Firebase 설정이 비어있습니다."))
        return runCatching {
            val (token, uid) = resolveIdentity(apiKey) ?: error("먼저 로그인을 해야 합니다.")
            val base = databaseUrl.trimEnd('/')
            val infoText = get(base, "groups/$groupId/info", token) ?: error("모임을 찾을 수 없습니다.")
            val info = JSONObject(infoText)
            if (info.optString("ownerUid") != uid) error("모임장만 소유권을 넘길 수 있습니다.")
            put(base, "groups/$groupId/info/ownerUid", token, JSONObject.quote(newOwnerUid))
            put(base, "groups/$groupId/admins/$uid", token, "true")
        }
    }
    ...
    fun findMemberPlantBadge(databaseUrl: String?, apiKey: String?, targetUid: String): PlantBadge? {
        if (databaseUrl.isNullOrBlank() || apiKey.isNullOrBlank()) return null
        return runCatching {
            val (token, _) = resolveIdentity(apiKey) ?: return@runCatching null
            val base = databaseUrl.trimEnd('/')
            ...
            for (groupId in myGroupIds) {
                val body = get(base, "groups/$groupId/stats/$targetUid", token) ?: continue
                val json = JSONObject(body)
