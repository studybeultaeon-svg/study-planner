package com.phonelock.desktop.calc
 ...
 * calcRequiredPace()/countDaysInRange()를 그대로 이식했다. 요일 인덱스는 JS Date.getDay()와
 * 동일하게 0=일 ~ 6=토를 쓴다(LocalDate.dayOfWeek는 1=월~7=일이라 변환 필요).
 */
object CalcEngine {

    fun jsDow(date: LocalDate): Int = date.dayOfWeek.value % 7 // MON(1)->1 ... SAT(6)->6, SUN(7)->0
    ...
    data class CalcResult(
        ...
        /** 페이스 부족 시에만 의미 있는, 마감을 맞추려면 필요한 요일별 목표. */
    ...
    /** "2025-01-01,2025-01-05" 형태 텍스트를 YYYY-MM-DD만 남기고 파싱(형식 다른 항목은 무시). */
    ...
    /** startDate부터 매일 dayGoals[dow]씩 진행 시 remaining이 소진되는 날짜(최대 10년 시뮬레이션). */
    ...
    /** endDate까지 remaining을 끝내려면 현재 dayGoals의 몇 배 페이스가 필요한지 역산. */
