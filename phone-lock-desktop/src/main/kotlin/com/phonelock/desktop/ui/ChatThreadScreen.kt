...
/** [com.phonelock.desktop.routine.ChatNotifier]의 새 메시지 알림 폴링이, 지금 사용자가 보고 있는
 *  대화방과 같은 대화방이면 중복 알림을 건너뛰기 위해 참조하는 전역 상태(2026-09-10, 채팅 알림 신규,
 *  안드로이드판 `ActiveChatTracker`와 대칭). */
...
/**
 * 1:1 DM(92차 소셜 개편 Phase 2, 안드로이드판 ChatThreadScreen.kt와 대칭)의 공용 메시지 스레드 UI —
 * 텍스트 + 이모지 리액션만. 실시간성은 "이 화면이 켜져있는 동안만"으로 확정돼 [POLL_INTERVAL_MS] 주기로
 * 폴링한다(CalendarScreen.DayDetailSection과 같은 패턴). 저장 경로(`dmChats`)는 콜백으로 주입한다.
 * [chatId]는 [ActiveChatTracker] 갱신용.
 */
@Composable
fun ChatThreadScreen(
    ...
    LaunchedEffect(Unit) {
        while (true) {
            applyLoadResult(withContext(Dispatchers.IO) { loadMessages() })
    ...
    fun sendCurrentInput() {
        ...
        scope.launch {
            ...
            val loaded = withContext(Dispatchers.IO) { loadMessages() }
