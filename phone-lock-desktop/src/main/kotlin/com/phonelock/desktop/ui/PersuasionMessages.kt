...
/**
 * 제한을 약화시키는 시도(설정 약화 저장, 그룹 삭제, 그룹 일정 끄기, 프로그램 종료 등)를 할 때
 * 하나씩 순서대로 보여주는 회유 멘트. 각 멘트마다 사용자가 직접 "예"를 눌러야 하고, 누르면 그때부터
 * 무작위 시간 대기한 뒤 자동으로 다음 멘트로 넘어간다(마지막 멘트는 실제 행동 실행). 눌러야
 * 시작되고, 시작되면 다시 못 누른다 — 미리 잠겨있다가 풀리는 방식이 아니다.
