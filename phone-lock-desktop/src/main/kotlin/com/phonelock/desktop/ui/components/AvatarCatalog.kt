...
/**
 * 프로필 사진(112차, 안드로이드판과 대칭) — 갤러리 업로드 대신 앱이 제공하는 동물 이모지 프리셋 중
 * 하나를 고른다. Firebase Storage 등 별도 저장 인프라 없이 `users/{uid}/profile.profileImage`에 이
 * id 문자열 하나만 저장하면 되므로, 현재 프로젝트 규모에 맞는 가장 단순한 방식이다.
 */
