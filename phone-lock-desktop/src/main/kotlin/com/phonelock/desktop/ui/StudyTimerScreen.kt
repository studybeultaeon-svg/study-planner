...
/**
 * 타이머를 끄지 않고 지금 재고 있는 공부 일정만 바꾸는 다이얼로그(126차, 사용자 요청, 안드로이드판과
 * 대칭) — 타이머 탭과 공부 잠금 화면([StudyLockScreen])이 같은 것을 쓴다. 시작 전 입력칸과 같은 규칙으로
 * "오늘 캘린더 일정 중에서 고르거나 직접 입력"이 둘 다 되게 한다. 확인을 누르면 호출부가
 * `timerChangeTask`를 부르고, 거기서 지금까지 잰 구간은 바꾸기 전 이름으로 기록에 적립된다.
 */
@Composable
