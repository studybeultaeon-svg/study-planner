...
- 빌드/배포: 안드로이드 컴파일/릴리스 빌드 확인 및 3위치(`AndroidBuilds`/OneDrive 원본/`vm-build-output`) 재배포 완료. 데스크탑은 컴파일 확인만 완료 — 호스트 실제 교체 배포는 "호스트에서 직접 빌드/배포하려면 매번 재확인" 원칙에 따라 사용자 확인 대기 중. GitHub 릴리스/공개 저장소 동기화도 보류 중.
