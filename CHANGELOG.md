...
### 1. 버그 수정 — 126차 이후 데스크탑에서 폰 공부기록이 안 보임(126차 수정의 회귀)
- **원인**: 126차가 옛 칸(플랫폼 이름 그대로인 `"android"`/`"desktop"`)을 **칸 이름만 보고 통째로 버리게** 했다. "각 기기가 한 번 더 기록하면 새 칸으로 다시 올라가 저절로 복구된다"는 전제였지만, 기기 업데이트 시점이 다르면 성립하지 않는다 — 실제로 폰은 아직 126차 이전 빌드라 `"android"` 칸에 올리는데 먼저 업데이트된 데스크탑이 그 칸을 버려서, 126차 전엔 잘 보이던 폰 기록이 데스크탑에서 사라졌다(실측: 그날 폰 기록 1236초가 데스크탑 기준 0초로 계산됨).
- **수정**: 칸 이름 대신 **기록 지문(`시작시각|길이|이름`)** 으로 걸러낸다 — 내 칸은 통째로 건너뛰고, 나머지 칸(옛 칸 포함)은 전부 읽되 로컬 기록과 지문이 겹치는 것만 뺀다. 업데이트 순서와 무관하게 남의 기록은 항상 보이고 내 기록은 이중 계산되지 않는다.
- 병합 규칙을 순수 함수 `mergeRemoteStudyLog()` / `studyLogFingerprint()`로 분리(양 플랫폼 동일).

### 2. 버그 수정 — 일일 사용한도가 안드로이드 기기 수만큼 느슨해짐(126차 Open 항목)
- **원인**: `dailyUsage/{날짜}/{그룹}/{기기}`의 기기 값이 아직 플랫폼 상수라 폰과 태블릿이 같은 칸을 서로 덮어썼고(마지막에 올린 기기 값만 남음), 읽을 땐 그 칸을 자기 것으로 보고 빼서 **같은 플랫폼 다른 기기의 사용시간이 한도 판정에서 통째로 빠졌다**. 실측: 오늘 모든 그룹이 `android` 한 칸만 사용 중.
- **수정**: 공부기록과 같은 `deviceSlotKey()`(= `android-<id>`/`desktop-<id>`)로 올리고, 읽을 땐 **내 칸만** 빼고 나머지를 전부 더한다(`peerUsageSecondsOf()`). 옛 칸은 아직 업데이트 안 된 기기가 쓰고 있을 수 있으므로 남겨서 합산한다 — 한도는 덜 세는 쪽이 위험하기 때문.
- **내 옛 칸 정리**: 앱 실행당 (날짜, 그룹)마다 한 번, 첫 push 직전에 옛 칸을 삭제한다(`deleteLegacyDailyUsage()`). 업데이트 전 내가 올려둔 값이 "남의 기기 몫"으로 오인돼 한도가 빨리 차는 것을 막는다.

### 리팩터링
- 양 플랫폼의 `studyLogDeviceKey()` → `deviceSlotKey()`로 이름 변경(공부기록과 일일 사용시간이 같은 키를 쓰게 됨).

### 검증
- 데스크탑 JUnit `DeviceSyncMergeTest` 5개 신설(영구) — 전체 22/22 통과. 126차 규칙으로 되돌린 사본에서 5개 중 3개 실패하는 역검증 완료.
- 안드로이드 컴파일 + 기존 유닛테스트 31/31 통과.
- 실제 Firebase 데이터로 확인: 오늘치 원격 기록에 126차 규칙은 0초, 127차 규칙은 1236초(20분).
- 릴리스: android `1789741813`(4곳 해시 일치) / desktop `1789741938`(호스트 교체·재실행, 3곳 jar 해시 일치), GitHub 릴리스 게시.
- **실기기 확인은 아직** — 폰·태블릿 2대 + 데스크탑이 함께 있어야 검증된다.

---

## 2026-09-18 (126차) — 완료 취소 시 경험치 회수 + 공부 잠금 화면 재배치 + 실행 중 일정 변경 + 기기별 공부기록 키

사용자 보고 4건(경험치 무한 복사 / 태블릿 잠금 화면에서 허용앱이 절반 차지 / 실행 중 일정 변경 불가 / 태블릿 공부기록이 다른 기기에 안 뜸)을 한 세션에서 처리.

### 1. 버그 수정 — 완료를 취소해도 경험치가 안 돌아옴(경험치 무한 복사)
- **원인**: 포인트 원장은 `revokePointsOnce()`가 되돌렸지만, 같은 적립에 편승했던 성장 EXP(`awardGrowthExp`)는 되돌리는 경로가 아예 없었다. 캘린더·루틴·스트릭 보너스가 전부 이 경로라 **루틴 완료 취소에서도 같이 발생**.
- **수정**: `revokeGrowthExp(rawAmount)` 신설(`Repository.Growth.kt` / `PhoneLockRepository.Growth.kt`) — 대기 EXP에서 먼저 빼고, 이미 "경험치 적용"으로 누적(`growthExpTotal`)에 넘어간 뒤라면 모자란 만큼 누적에서 마저 뺀다(둘 다 0 미만으로 안 내려감). `revokePointsOnce()`가 지우는 원장 항목의 실제 `delta`를 읽어 그 양만큼 호출한다(상수 대신).
- **왜 누적에서도 빼는가**: 대기에서만 빼면 "완료 → 적용 → 취소"를 반복하는 것만으로 그대로 무한 복사가 된다([[DECISIONS.md]] 126차).

### 2. 공부 잠금 화면 — 허용 앱을 아래 절반 목록에서 플로팅 다이얼로그로
- 위 `weight(1.1f)` / 아래 `weight(1f)` 2단 분할을 없애고 **타이머 한 컬럼이 화면 전체**를 쓴다. 허용 앱/프로그램은 하단 `📱 허용된 앱 (N)` 버튼 → `AlertDialog` 안의 기존 `AllowedAppsFlow`로 이동.
- 양 플랫폼 동일(`StudyLockActivity.kt` / `StudyLockScreen.kt`).

### 3. 타이머 실행 중 공부 일정 변경
- `timerChangeTask(newTaskName)` 신설(양 플랫폼 Repository) — 지금까지 잰 구간은 **바꾸기 전 이름으로** 기록에 적립하고 새 이름으로 구간을 다시 시작한다. 페이즈(`phaseStartedAt`/`phaseEndAt`/`cycleCount`)는 건드리지 않아 뽀모도로 카운트다운과 스톱워치 표시가 끊기지 않는다.
- 이를 위해 `TimerRunState.taskStartedAt`(적립 구간 시작) 필드 추가 — 기존 `phaseStartedAt`이 겸하던 "적립 구간 시작" 역할을 분리. `timerStop`/`timerSwitchPhase`도 이 값을 쓰도록 변경. 안드로이드는 `AppPreferences.timerTaskStartedAt`, 데스크탑은 `JsonStore`의 `timerRun.taskStartedAt`에 저장. 0이면 `phaseStartedAt`으로 대체(구버전에서 켜둔 채 업데이트된 타이머 호환).
- UI: 공용 `StudyTaskChangeDialog`(오늘 캘린더 일정 목록 + 자유 입력) — **타이머 탭**(실행 중 업무명 옆 "일정 변경")과 **공부 잠금 화면**(하단 `📖 일정 변경`) 양쪽에서 쓴다. 다른 기기 세션을 미러링 중이거나 원격 신호로 잠긴 경우엔 안 보인다(정지/전환과 같은 규칙).
- 타이머 탭의 오늘 일정 목록을 실행 중에도 5초 주기로 갱신하도록 변경(다이얼로그가 최신 목록을 보여줘야 해서).

### 4. 버그 수정 — 태블릿에서 공부해도 폰/데스크탑에 기록이 안 뜸
- **원인**: 공부 기록을 올리는 Firebase 칸 이름이 플랫폼 상수 하나(`"android"`/`"desktop"`)였다. 폰과 태블릿이 **같은 칸에 서로 덮어쓰고**, 읽을 땐 그 칸을 자기 것으로 보고 건너뛰어 **안드로이드끼리는 서로의 기록을 영영 못 봤다**. 데스크탑은 태블릿 기록을 볼 수 있었지만 폰이 다음 기록을 올리는 순간 덮여 사라졌다.
- **수정**: 설치본마다 하나씩 뽑아 저장하는 `deviceInstallId`(안드로이드 `AppPreferences`, 데스크탑 `AppData`)를 붙여 `android-<id>` / `desktop-<id>`를 칸 이름으로 쓴다. 읽을 땐 자기 칸과 **플랫폼 이름 그대로인 옛 칸**을 건너뛴다(누구 것인지 알 수 없고 자기 기록이 섞여 있으면 로컬과 이중 합산됨 — 각 기기가 그날 한 번 더 기록하면 그날치 전체가 새 칸으로 올라가 저절로 복구).
- 일일 사용시간(`dailyUsage`)은 이번에 손대지 않았다 — 같은 뿌리의 문제가 남아 있음([[BUGS.md]] 126차 Open).

### 검증
- **데스크탑 JUnit(임시 스크래치 테스트, 커밋 안 함)**: 캘린더 완료 취소/O→X 전환/루틴 완료 취소에서 대기 EXP 원복, "완료→적용→취소" 5회 반복 후 누적 EXP 제자리, `timerChangeTask` 후 앞 구간이 이전 이름으로 1건 기록되고 `phaseStartedAt` 불변 — 4/4 통과. **역검증**: `revokeGrowthExp` 호출만 주석 처리한 사본에서 3/4 실패(테스트가 실제로 이 버그를 잡는지 확인).
- **안드로이드 실기 시나리오(Android 16 에뮬레이터, 릴리스 APK)**: 캘린더 완료 → `growth_exp_pending=5.0`, 취소 → `0.0`. "경험치 적용"으로 `growth_exp_total=5.0` 만든 뒤 취소 → `0.0`(적용분까지 회수), 완료/취소 재반복해도 0 유지.
- 태블릿(1600×2560 @240dpi)·가로 태블릿(2560×1600)·폰(1080×2400) 3가지 크기에서 공부 잠금 화면이 **스크롤 없이** 한 화면에 들어오는 것 확인, 허용 앱 다이얼로그 동작 확인.
- 타이머 탭·잠금 화면 양쪽에서 일정 변경 → 업무명만 바뀌고 스톱워치는 계속 흐르며, 앞 구간이 이전 이름으로 "오늘의 공부 기록"에 남는 것 확인.
- 기존 데스크탑 JUnit 12/12 통과, 안드로이드 `compileReleaseKotlin`/`assembleRelease` 성공.
- 릴리스: android `1789733126`(3곳 해시 일치) / desktop `1789733044`(호스트 교체·재실행, jar 해시 3곳 일치), GitHub 릴리스 게시.
- **실기기(폰·태블릿 동시) 확인은 아직** — 특히 4번은 기기 2대가 있어야 검증된다.
...
- **감사**: P0 8건 / P1 16건 / P2 11건 / P3 7건 도출, 종합 판정 "출시 보류". 주요 P0 — 자체 APK 업데이트(Play 정책 위반), 관리자 승인 게이트(신규 사용자 100% 무한 대기), targetSdk 34(등록 불가), 개인정보처리방침 부재, 접근성+기기관리자 미선언, 무전기 음성 폴링(메시지 1건당 하루 최대 약 2.6GB), Room 파괴적 마이그레이션 + 28→29 누락, 6자리 초대 코드. 보고서 아티팩트: https://claude.ai/code/artifact/169559f1-c6d2-4ebf-8697-529695082db0
- **분리**: 이 프로젝트를 출시용으로 개조하는 대신 `C:\Projects\godsaeng-release`에 복사본을 만들어 독립 프로젝트로 분리했다. 개인용(이 프로젝트)은 기존 방향성을 그대로 유지한다.
- **분리된 프로젝트에서 바뀐 것(여기에는 적용하지 않음)**: applicationId `com.beultaeon.studyloop`, 앱 이름 "회독플랜", 데스크탑 데이터 폴더 `StudyLoopDesktop`, 로컬 API 포트 47883, Firebase 전용 프로젝트(미생성, placeholder), 제품 방향 "회독 자동화 + 딴짓 차단".
- **작업 중 사고 1건(복구 완료)**: 문서 갱신 스크립트가 임시 파일을 못 찾아 중단되면서 `HANDOFF.md`가 0바이트로 잘렸다. 즉시 `git restore`로 복구(117,043바이트, 커밋 `76414a0` 상태와 동일)했고 코드에는 영향이 없었다. **교훈**: 파일을 덮어쓰는 스크립트는 쓰기 전에 모든 내용을 메모리에서 먼저 완성하고, 임시 파일은 호출 간에 사라질 수 있는 `/tmp` 대신 세션 스크래치패드를 쓸 것.

---

## 2026-09-13 (118차) — 홈 화면 정보-표기 3종: 다음 일정 미리보기 + 레벨업 EXP % + 주간 완료율 미니 스파크라인

사용자 요청: 117차 세션 말미에 후보로만 기록해둔 "홈 화면 추가 정보-표기 아이디어" 5개 중 3개를 골라 구현(다음 캘린더 일정 미리보기/레벨업까지 남은 EXP %/이번 주 완료율 미니 그래프 — 오늘 남은 공부 목표 진행률 링과 장식 아이템 툴팁은 이번엔 제외). 양 플랫폼(`PlantScreen.kt`) 동일 적용:

- **다음 캘린더 일정 미리보기**: 완료(O) 안 된 일정 중 오늘 이후로 가장 가까운 것 하나를 좌상단 미니 요약 칩에 "📅 제목 (D-N)" 형식으로 표시(`getAllCalendarTasks`/`getAllCalendarTasksOnce` + `dateKey` 오름차순 `minByOrNull`).
- **레벨업까지 남은 EXP %**: 레벨/경험치 HUD의 진행바 아래에 "다음 레벨까지 N%" 텍스트 추가(`(1 - levelProgress) * 100`, 최대 레벨이면 숨김) — 기존엔 진행바만 있어 정확한 퍼센트를 알 수 없었음.
- **이번 주 완료율 미니 스파크라인**: 최근 7일(오늘 포함) 일별 루틴 완료율을 좌상단 미니 요약 칩에 얇은 막대 7개로 표시 — 루틴 통계 탭의 30일 막대그래프와 같은 색상 규칙(100%=초록/일부=노랑/0%=빨강/예정 없음=회색)의 축소판.
- **빌드/배포**: 양 플랫폼 컴파일 확인 → `assembleRelease`(versionCode `1789267923`)/`packageMsi createDistributable`(BuildInfo `1789268044`) 정상 통과. 안드로이드 APK 3위치 해시 일치 배포, 데스크탑 호스트(`PhoneLockDesktopApp`, watchdog 재기동 경합으로 robocopy 1차 시도 실패 후 프로세스 재종료+재시도로 성공)+`vm-build-output` 재배포+재기동, GitHub 릴리스 게시(`android-1789267923`, `desktop-1789268044`), `sync-public-repo.ps1`로 공개 저장소 push까지 완료. **실사용 검증은 안 됨**(다음 일정 D-day 계산이 정확한지, EXP % 텍스트가 진행바와 일치하는지, 스파크라인 7일 색상이 실제 루틴 완료 이력과 맞는지 확인 필요).
- IDEAS.md "홈 화면 추가 정보-표기 아이디어"(117차) 항목에서 구현한 3개를 취소선 처리, 남은 2개(공부 목표 진행률 링/장식 아이템 툴팁)는 계속 후보로 유지.
...
- **빌드/배포**: `compileDebugKotlin`/`compileReleaseKotlin`/`assembleRelease` 전부 정상 통과(신규 컴파일 에러 없음, 기존 경고만 남음). versionCode `1789187642`로 안드로이드 릴리스 APK를 3위치(`AndroidBuilds\phone-lock-app-release.apk`, OneDrive 원본, `vm-build-output\android\app-release.apk`)에 배포, `sync-public-repo.ps1`로 공개 저장소 동기화 + GitHub 릴리스(`android-1789187642`) 게시까지 완료. **실사용 검증만 남음.**
...
- **설정 탭 권한 UI 통합**: `SettingsScreen.kt`(안드로이드)에서 접근성/배터리 최적화/정확한 알람/삭제 방지 4개를 따로 나열하던 "권한 / 백그라운드 보호" 카드를 요약 상태 한 줄 + "권한 설정 가이드 열기" 버튼 하나로 축소, 버튼을 누르면 온보딩 때와 같은 `PermissionOnboardingScreen`을 그대로 재사용해서 연다(닫으면 설정 화면으로 복귀). 데스크탑은 애초에 이런 OS 런타임 권한 개념이 없어 변경 대상 아님.
- **관리자 패널 "게스트" 오판정 버그 수정**: 원인은 안드로이드 `AccountGateScreen.kt`의 가입 신청 제출 코드가 실제 로그인 방식(`AuthManager.currentUser?.isAnonymous`)과 무관하게 서버에 `isGuest = true`를 하드코딩해서 보내고 있었던 것 — 회원가입/정상 로그인 사용자까지 전부 "게스트"로 저장됐다(데스크탑은 이 버그 없음, 처음부터 올바르게 구현돼 있었음). 지역변수 `isGuest`를 그대로 넘기도록 한 줄 수정. **하위호환**: 이미 잘못 저장된 기존 승인 사용자는 다음 로그인 시(`AccountGate`가 "approved" 상태를 받을 때마다) 현재 세션의 실제 `isAnonymous` 값과 저장된 값이 다르면 자동으로 서버 값을 교정하는 `AccountSyncClient.fixGuestFlagIfNeeded()`를 추가해 자연스럽게 정정되도록 함 — 관리자가 직접 고쳐줄 필요 없음, 각 사용자가 스스로 로그인할 때 자가 치유.
- **온라인/오프라인 모드 전환 UI 노출 범위 수정**: 판정 로직(`isEffectivelyOffline()`)은 98차부터 이미 게스트(익명 계정)만 포함하도록 정확했지만, 설정 탭의 수동 전환 스위치("온라인 / 오프라인 모드" 카드)는 로그인 방식과 무관하게 모든 사용자에게 노출되고 있었음 — 양 플랫폼 `SettingsScreen.kt`에서 `AuthManager.currentUser?.isAnonymous == true`(안드로이드) / `AuthManager.isAnonymous`(데스크탑) 조건으로 게스트에게만 보이도록 감쌌다.
- **루틴 탭 이모지 불일치 정리**: 루틴 탭/서브탭 아이콘은 이미 📋(102차 이후)인데, 루틴 화면 헤더("🌱 루틴")·루틴 스트릭 알림 제목·홈스크린 위젯 날짜 라벨·소셜 멤버 상세의 "루틴" 서브탭 라벨에는 여전히 옛 🌱 표기가 남아있었다 — 🌱는 105차부터 별도 "식물" 탭 전용 의미로 굳어졌으므로 이 5곳(안드로이드 4곳+데스크탑 1곳, 총 8개 파일) 전부 📋로 통일. 소셜 탭의 "아직 속한 모임이 없습니다" 빈 상태 일러스트(🌱)는 루틴/식물과 무관한 장식용이라 그대로 둠.
- **불안정한 네트워크에서 오인 로그아웃 버그 수정(데스크탑)**: 사용자 제보 — "정상 로그인 상태인데 인터넷이 불안정하면 의도치 않게 로그아웃된 것처럼 보인다". 코드 추적 결과 양 플랫폼 어디에서도 네트워크 실패를 원인으로 `AuthManager.signOut()`이 자동 호출되는 경로는 없었음(모든 `signOut()` 호출은 사용자가 직접 로그아웃 버튼을 누른 경우뿐, 토큰 갱신 실패도 해당 요청 하나만 실패시킬 뿐 세션 자체는 건드리지 않음). 실제 원인은 데스크탑 `AccountGateScreen.kt`의 `refreshProfile()` — 프로필 조회가 네트워크 오류로 실패해도 `serverChecked = true`를 무조건 세팅해버려서, 이미 승인된 사용자가 7초 폴링 중 단 한 번만 실패해도 "낙관적으로 계속 보여주기" 조건(`!serverChecked && optimisticApproved`)을 벗어나고 `serverStatus`는 여전히 null이라 곧장 가입 신청 화면(`UsernameStep`)으로 떨어져 로그아웃된 것처럼 보였다(실제 Firebase 세션/로컬 `google_auth.json`은 전혀 안 건드림). 안드로이드의 동일 지점(`refreshFromServer()`)은 이미 "낙관적 승인 상태거나, 네트워크 오류일 때는 상태를 안 바꾼다"로 올바르게 구현돼 있었음 — 데스크탑을 그 패턴에 맞춰 "이미 승인된 걸 알고 있었다면(캐시든 이전 성공 확인이든) 이번 실패로 그 상태를 지우지 않는다"로 수정. 온라인/오프라인 모드 기능 자체는 이번 수정과 무관(사용자가 우회 대신 근본 원인 수정을 요청함).
- 빌드/배포: 안드로이드 컴파일/릴리스 빌드 확인 및 3위치(`AndroidBuilds`/OneDrive 원본/`vm-build-output`) 재배포 완료. 데스크탑은 사용자 확인을 받아 호스트 실제 교체 배포까지 완료(배포 도중 78차에 문서화된 "워치독이 즉시 재기동해 robocopy가 exe를 못 옮기는" 증상이 재현됐고, 문서에 적힌 대로 Stop-Process 재실행 후 즉시 재시도해서 해결 — 배포된 jar에 이번 수정 코드(`hadKnownApproval`)가 실제로 들어있는지 압축 해제해 확인함). GitHub 릴리스(`android-1789118943`/`desktop-1789119212`) 게시 및 공개 저장소(`study-planner`) push까지 완료.
- **실사용 검증 필요**: 위 "다음 작업 우선순위" 107차 항목 참고. 특히 오인 로그아웃 버그는 코드 추적으로 원인은 확정했지만 실제로 인터넷을 끊었다 붙였다 하는 환경에서의 재현 검증은 이번 세션에서 못 했음(빌드 환경에 실제 네트워크 단절을 재현할 장치가 없었음).

---

## 2026-09-11 (106차 세션) — 안드로이드/태블릿 당겨서 새로고침 아이콘 고착 버그 수정 + 소셜 탭 메인 화면 전체 스크롤

- **당겨서 새로고침 아이콘이 안 사라지는 버그(99차부터 이월, 원인 미확정이었음)**: 원인은 material3 1.2.x의 실험적 `rememberPullToRefreshState`+`PullToRefreshContainer` 조합이 드래그가 임계값을 살짝 못 넘긴 채 손을 뗐을 때 인디케이터가 완전히 수축하지 못하고 화면에 남는 라이브러리 자체 버그였음. `compose-bom`을 2024.06.00→2024.09.00(material3 1.3.0), Kotlin을 1.9.24→1.9.25, compose 컴파일러 확장을 1.5.14→1.5.15로 올려 `PullToRefreshBox.kt`(공용 컴포넌트, 타이머/캘린더/계산기/일정표/통계 등 "공부" 섹션 전 화면과 루틴/그룹/모임 목록이 공유)를 material3 1.3.0의 안정화된 `PullToRefreshBox` API로 교체 — `isRefreshing`/`onRefresh`만 넘기면 인디케이터 표시·수축을 라이브러리가 전부 관리해 이 종류의 버그 자체가 구조적으로 사라짐. 공개 API(함수 시그니처)는 그대로 유지해 호출부(10개 화면) 수정 없음.
- **소셜 탭 메인 화면 스크롤 추가**: `SocialGroupScreen.kt`가 위쪽(1:1 대화 목록+헤더)은 스크롤 밖이고 아래쪽 모임 목록만 `LazyColumn`으로 자체 스크롤해서, 모임/대화가 많으면 화면 위쪽이 잘려 안 보이던 문제 — 전체를 하나의 `Column.verticalScroll`로 통일하고 모임 목록은 `LazyColumn`에서 일반 `Column`+`forEach`로 교체(스크롤 중복 방지, 목록 길이가 짧아 지연 로딩 불필요).
- 빌드/배포: 안드로이드 컴파일+릴리스 빌드 확인, 3위치(`AndroidBuilds`/OneDrive 원본/`vm-build-output`) 재배포, GitHub 릴리스(`android-1789117557`) 게시, 공개 저장소(`clean-main`→`main`) push까지 완료. 데스크탑 쪽 변경 없음(이번 버그/요청 둘 다 안드로이드/태블릿 전용).
- **실사용 검증 필요**: 실제 기기에서 당겨서 새로고침 후 스피너가 매번 확실히 사라지는지(특히 임계값 근처에서 살짝 당겼다 뗀 경우), 소셜 탭에서 모임/대화가 많을 때 위아래로 전체 스크롤이 자연스러운지.

---

## 2026-09-11 (105차 세션, 버그 수정) — 안드로이드 릴리스가 빌드 경로 오타로 구버전 그대로 배포된 문제 수정

- 105차 3차 개편(아래) 직후 게시한 `android-1789051210` 릴리스가, 소스 동기화 robocopy가 `C:\AndroidBuilds\phone-lock-android`(캐노니컬 경로 `C:\Users\sunae\AndroidBuilds\phone-lock-android`와는 다른 실제 디렉터리)로 잘못 향해 실제로는 옛 소스 그대로 빌드된 구버전이었음 — 사용자가 자체 업데이트로 설치해보니 식물 탭 자체가 안 보인다고 제보해서 발견. 상세 원인/재발 방지는 [[BUGS.md]] 참고.
- 올바른 경로로 재동기화 → 재빌드 → `unzip`+`grep -a`로 APK의 `classes.dex`에 신규 문자열(`growth_exp_total`/`jinseok_tralalero` 등)이 실제로 포함됐는지 검증(이번엔 확인 후 배포) → 3위치 재배포 → 잘못된 GitHub 릴리스(`android-1789051210`) 삭제 후 올바른 릴리스(`android-1789082482`) 게시.
