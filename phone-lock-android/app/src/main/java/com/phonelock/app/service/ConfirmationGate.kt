...
/**
 * 그룹 단위로 "예"를 선택한 뒤 짧은 시간 동안 같은 그룹 내 다른 앱/사이트도 다시 확인하지 않도록 하는
 * 인메모리 쿨다운. AccessibilityService는 단일 프로세스에서 계속 실행되므로 메모리 저장으로 충분하다.
 */
object ConfirmationGate {
    ...
    /** cooldownSeconds: 그룹별로 설정하는 "재확인까지 유예시간". */
    @Synchronized
    ...
    /** 다음 실행 확인이 다시 뜨기까지 남은 시간(초). 확인 기록이 없거나 이미 유예시간이 지났으면 0. */
    @Synchronized
