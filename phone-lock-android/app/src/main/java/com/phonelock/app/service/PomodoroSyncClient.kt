...
object PomodoroSyncClient {
     ...
     * 페이즈에선 false). 이 값이 true인 동안 [com.phonelock.app.service.AppMonitorAccessibilityService]가
     ...
     * 매초 아님)마다 `users/{user}/pomodoro`에 write해서 다른 기기(데스크탑↔모바일)에도 신호를 준다.
     * 이 기기 자신의 공부 잠금 판정은 더 이상 이 값을 읽지 않고 로컬(PhoneLockRepository)을 직접
     * 읽는다 — DECISIONS.md "공부앱을 웹/웹뷰가 아닌 완전 네이티브로 재구현" 참고. 실패해도 로컬
    ...
    suspend fun writeDailyUsage(databaseUrl: String?, apiKey: String?, date: String, groupName: String, device: String, usedSeconds: Int) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                conn.outputStream.use { it.write(usedSeconds.toString().toByteArray()) }
                conn.responseCode
                conn.disconnect()
            }
        }
    ...
    /**
     * 126차 이전 빌드가 쓰던 옛 칸(플랫폼 이름 그대로, 예: `dailyUsage/{date}/{group}/android`)을 지운다 —
     ...
     * 다시 올리므로 저절로 복구된다.
     */
    suspend fun deleteLegacyDailyUsage(databaseUrl: String?, apiKey: String?, date: String, groupName: String, legacyDevice: String) {
        if (databaseUrl.isNullOrBlank() || apiKey.isNullOrBlank()) return
        withContext(Dispatchers.IO) {
            runCatching {
                val (token, user) = resolveIdentity(apiKey) ?: return@runCatching
                val base = databaseUrl.trimEnd('/')
                val url = URL("$base/users/$user/dailyUsage/$date/${firebaseSafeKey(groupName)}/$legacyDevice.json?auth=$token")
                val conn = (url.openConnection() as HttpURLConnection).apply {
    ...
    suspend fun writeStudyLogForDate(databaseUrl: String?, apiKey: String?, dateKey: String, device: String, entries: org.json.JSONArray) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                conn.outputStream.use { it.write(entries.toString().toByteArray()) }
                conn.responseCode
                conn.disconnect()
            }
        }
    ...
    /**
     * 네이티브 계산기(3단계) 전체 문서를 읽는다. 웹앱과 완전히 같은 경로/스키마(`users/{user}/calculator`)를
    ...
    /**
     * 부분 갱신(PATCH) 전송 — Android `HttpURLConnection`은 PATCH를 직접 지원하지 않아(허용 메서드 목록에
     * 없음) POST + `X-HTTP-Method-Override: PATCH` 헤더로 우회한다(Firebase REST API가 공식 지원하는
     * 방식). PUT을 쓰면 이 함수가 모르는 다른 키(폴더 트리 등)까지 지워지므로 반드시 이 경로여야 한다.
     */
    ...
    suspend fun writeCalcTasksAndSaved(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                sendPatch(URL("$base/users/$user/calculator.json?auth=$token"), body)
            }
        }
    }
    ...
    private fun fetchStatus(databaseUrl: String, user: String, idToken: String): JSONObject? = runCatching {
        val base = databaseUrl.trimEnd('/')
        val url = URL("$base/users/$user/pomodoro.json?auth=$idToken")
        val conn = (url.openConnection() as HttpURLConnection).apply {
            requestMethod = "GET"
            connectTimeout = TIMEOUT_MS
            readTimeout = TIMEOUT_MS
