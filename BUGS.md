...
## Fixed (2026-09-18, 126차) — 완료를 취소해도 경험치가 안 돌아옴(경험치 무한 복사)

- **설명**: 캘린더 일정이나 루틴을 "완료"했다가 취소해도 그때 적립된 성장 EXP가 그대로 남았다. 완료/미완료를 반복하면 EXP를 무한히 불릴 수 있었고, "경험치 적용"으로 레벨에 반영한 뒤 취소하면 누적 EXP까지 영구히 남았다(사용자 보고).
- **원인**: 포인트 원장은 `revokePointsOnce()`가 되돌렸지만, 같은 적립에 편승하던 `awardGrowthExp()`에는 대응하는 회수 경로가 아예 없었다. 캘린더·루틴·스트릭 보너스가 전부 이 경로를 쓴다.
- **상태**: Fixed
- **해결**: `revokeGrowthExp(rawAmount)` 신설(양 플랫폼 `*.Growth.kt`) — 대기 EXP에서 먼저 빼고 모자라면 누적 EXP에서 마저 뺀다(0 미만 불가). `revokePointsOnce()`가 지우는 원장 항목의 실제 `delta`만큼 호출. 왜 누적에서도 빼는지는 [[DECISIONS.md]] 126차.
- **검증**: 데스크탑 JUnit 스크래치 테스트 4/4 통과(+ `revokeGrowthExp` 호출만 뺀 사본에서 3/4 실패하는 역검증), 안드로이드 에뮬레이터에서 `growth_exp_pending`/`growth_exp_total` 값 직접 확인. 실기기 미확인.

---

## Fixed (2026-09-18, 126차) — 태블릿에서 공부해도 폰/데스크탑에 공부 기록이 안 뜸

- **설명**: 태블릿으로 공부한 시간이 스마트폰과 데스크탑의 공부 기록에 나타나지 않았다(사용자 보고). 타이머 실시간 미러링("다른 기기에서 실행 중")은 정상이었고 **기록만** 누락됐다.
- **원인**: 공부 기록을 올리는 Firebase 칸 이름(`studyLog/{날짜}/{기기}`)이 플랫폼 상수 하나(`DAILY_USAGE_DEVICE` = `"android"`/`"desktop"`)였다. 폰과 태블릿이 같은 칸에 각자 그날치 전체를 PUT해 서로 덮어썼고, 읽을 땐 그 칸을 자기 것으로 보고 건너뛰어 안드로이드끼리는 서로의 기록을 아예 못 봤다. 데스크탑은 태블릿 기록이 보였지만 폰이 다음 기록을 올리는 순간 덮여 사라졌다.
- **상태**: Fixed
- **해결**: 설치본별 `deviceInstallId`를 붙인 `android-<id>`/`desktop-<id>`를 칸 이름으로 사용. 읽을 땐 자기 칸과 옛 플랫폼 칸을 건너뛴다. 설계 근거·옛 칸 처리는 [[DECISIONS.md]] 126차.
- **검증**: 컴파일/빌드/릴리스 완료. **기기 2대 동시 실사용 확인은 아직** — 이 버그는 폰과 태블릿이 함께 있어야 검증된다.

---

## Open (2026-09-18, 126차) — 일일 사용시간도 같은 이유로 안드로이드 기기끼리 서로 덮어씀

- **설명**: 위 공부 기록 버그와 같은 뿌리. `dailyUsage/{날짜}/{그룹}/{기기}`의 기기 값도 여전히 플랫폼 상수라, 폰과 태블릿이 같은 칸에 서로 덮어쓰고 상대 기기의 사용시간을 "다른 기기 합산"에서 빼먹는다. 결과적으로 **일일 사용한도가 안드로이드 기기 수만큼 느슨해질 수 있다**(태블릿에서 1시간 써도 폰의 한도 계산엔 안 들어감).
- **원인**: 126차에 공부 기록만 기기별 칸으로 바꾸고 `dailyUsage`는 그대로 뒀다.
- **상태**: Open — 사용자 확인 대기. 차단 한도 판정에 직접 쓰이는 값이라 잘못 건드리면 한도가 느슨해지는 방향으로 틀어질 수 있어 이번 범위에서 의도적으로 제외했다([[DECISIONS.md]] 126차 "범위"). 고칠 땐 공부 기록과 같은 방식(기기별 칸 + 옛 칸 건너뛰기)을 쓰되, 전환 당일 옛 칸을 무시하면 그날 한도가 한 번 느슨해진다는 점을 감안할 것.

---

## Fixed (2026-09-17, 125차) — 안드로이드 잠김·실행확인 화면이 이전 요청(다른 앱/그룹)의 내용을 계속 보여줌

- **설명**: 잠김(`BlockActivity`)/실행확인(`ConfirmOpenActivity`) 화면을 버튼 대신 홈 제스처로 벗어나 화면이 백그라운드에 남아 있으면, 다음에 다른 앱이 막혀도 이전 앱의 문구·사유가 그대로 보였다. 실행확인 화면은 "진행"을 누르면 이전 요청의 그룹이 확인 처리될 수 있었다.
- **원인**: 두 액티비티가 `launchMode="singleInstance"`라 새 요청이 `onNewIntent`로만 전달되는데, 이를 처리하지 않아 `onCreate`에서 만든 화면이 그대로 남았다.
- **상태**: Fixed
- **해결**: `IntentExtras.isSameLockRequest()`로 앱/사유/사이트/그룹을 비교해 다른 요청이면 `setIntent`+`recreate()`. 같은 요청이 겹쳐 오면 화면을 그대로 둔다(대기시간은 인텐트 전달 전후 일시정지 때문에 기존 규칙대로 어차피 초기화됨). `ConfirmationGate` 등 판정 로직은 건드리지 않음.
- **검증**: Android 16 에뮬레이터(스크래치 테스트 빌드)에서 수정 전 재현 → 수정 후 새 요청 기준으로 다시 그려짐 확인. 실기기 미확인.

---

## Open (2026-09-17, 125차 관찰) — `AppMonitorAccessibilityService.kt` 주석 한글이 깨져 있음

- **설명**: 파일 주석 약 95줄이 UTF-8 한글을 CP949로 잘못 읽었다가 다시 저장한 형태(`怨듬?` 등)로 깨져 있다. 코드 안 문자열(12개)은 정상이라 동작·화면에는 영향 없음.
- **원인**: 과거 어느 세션의 인코딩 처리 실수로 추정(파일은 UTF-8 BOM).
- **상태**: Open(낮음) — git 이력(121차 `1296f46` 이전)에서 정상 주석을 찾아 복원하는 별도 작업 필요.

---

## Fixed (2026-09-18, 126차) — 폰 크기에서 공부 잠금 화면 위쪽 영역의 "타이머 정지" 버튼이 경계에 걸려 반쯤 가려짐

- **설명**: 1080×2400(420dpi) 에뮬레이터에서 공부 잠금 화면 위쪽 영역(96차에 세로 스크롤을 넣은 부분)의 끝에 "타이머 정지" 버튼이 걸쳐 아랫부분이 잘려 보였다. 스크롤하면 전부 보였다. 태블릿에서는 같은 배치 때문에 아래 절반을 허용 앱 목록이 차지해 더 심했다(사용자 보고).
- **원인**: 위/아래 영역이 `weight(1.1f)`/`weight(1f)`로 높이를 나눠 갖는데 위쪽 내용이 그보다 약간 길었다.
- **상태**: Fixed
- **해결**: 126차에 아래 영역(허용 앱 목록)을 플로팅 다이얼로그로 옮기면서 2단 분할 자체를 없앴다 — 타이머 컬럼이 화면 전체를 쓰고 내용이 가운데 정렬된다.
- **검증**: 폰(1080×2400)·세로 태블릿(1600×2560 @240dpi)·가로 태블릿(2560×1600) 3가지 크기에서 스크롤 없이 전부 보이는 것 확인.
- **남은 별개 항목**: `StudyLockActivity`도 `singleInstance`인데 `onNewIntent` 처리가 없어 허용 앱 목록이 바뀐 요청이 오면 이전 목록이 남을 수 있음(미재현, 미수정).
