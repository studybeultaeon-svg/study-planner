...
- **설명**: 잠김(`BlockActivity`)/실행확인(`ConfirmOpenActivity`) 화면을 버튼 대신 홈 제스처로 벗어나 화면이 백그라운드에 남아 있으면, 다음에 다른 앱이 막혀도 이전 앱의 문구·사유(125차부터는 음악 카드까지)가 그대로 보였다. 실행확인 화면은 "진행"을 누르면 이전 요청의 그룹이 확인 처리될 수 있었다.
...
## Open (2026-09-17, 125차 관찰) — 폰 크기에서 공부 잠금 화면 위쪽 영역의 "타이머 정지" 버튼이 경계에 걸려 반쯤 가려짐

- **설명**: 1080×2400(420dpi) 에뮬레이터에서 공부 잠금 화면 위쪽 영역(96차에 세로 스크롤을 넣은 부분)의 끝에 "타이머 정지" 버튼이 걸쳐 아랫부분이 잘려 보인다. 스크롤하면 전부 보인다.
- **원인**: 위/아래 영역이 `weight(1.1f)`/`weight(1f)`로 높이를 나눠 갖는데 위쪽 내용이 그보다 약간 길다. 125차 음악 카드는 아래쪽 영역에 들어가 이 비율과 무관(수정 전부터 있던 배치).
- **상태**: Open(낮음, 관찰만) — 사용자 확인 후 비율/여백 조정 검토. `StudyLockActivity`도 `singleInstance`인데 `onNewIntent` 처리가 없어 허용 앱 목록이 바뀐 요청이 오면 이전 목록이 남을 수 있음(미재현, 같이 검토).
