...
### 소셜 채팅을 쳐서 보내도 메시지가 안 올라감 (96차 "엔터 전송" 수정 후에도 재발, 98차 원인 미확정)
- **경위**: 96차에 "엔터를 눌러도 안 올라간다"는 제보를 받아 키보드 전송 액션(`imeAction=Send`) 연결 누락으로 진단·수정했는데, 98차에 사용자가 "아직도 안 고쳐졌다"고 재차 제보 — 전송 버튼(➤) 클릭으로도 안 되는 것으로 보여 96차 진단이 근본 원인이 아니었을 가능성.
- **98차 조사**: `ChatSyncClient.sendGroupMessage`/`readGroupMessages`(양 플랫폼) 코드와 `firebase-database.rules.json`의 `groupChats`/`dmChats` 규칙을 정적으로 검토했으나 논리적으로 이상한 부분을 찾지 못함(멤버십 조건, senderUid 매칭 등 전부 일관돼 보임). 라이브 Firebase 자격증명 파일(`%APPDATA%\PhoneLockDesktop\data.json`)을 읽어 REST로 직접 확인하려 했으나 세션 권한 정책상 자격증명 파일 읽기가 차단됨.
- **98차 조치(우회, 근본 수정 아님)**: `ChatThreadScreen.kt`(양 플랫폼)가 `sendMessage`의 `Result<Unit>` 실패를 그동안 완전히 무시하고 있던 걸 발견 — 실패 사유를 화면에 그대로 표시하도록 계측 추가(`sendGroupChatMessage`/`sendDmChatMessage` 자체는 이미 `Result<Unit>`를 반환하고 있었음, UI가 안 읽었을 뿐).
- **의심 지점(미확인)**: 93차에 사용자가 Firebase 콘솔에 규칙을 수동으로 붙여넣었다고 기록돼 있는데, 이후(94~97차) 규칙 파일이 리포에서 편집됐다면 콘솔이 그걸 다시 게시받지 못했을 수 있음 — 98차 세션에 리포의 `firebase-database.rules.json` 전체를 사용자에게 파일로 전달, 콘솔에 통째로 재게시해달라고 요청함.
- **다음 단계**: 사용자가 다시 시도해서 화면에 뜨는 실제 에러 메시지를 확인하는 것이 가장 빠른 진단 경로.
