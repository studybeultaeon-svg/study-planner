...
### 안드로이드 당겨서 새로고침이 스크롤과 겹쳐서 잘 안 됨 (99차, 원인 미확정)
- **경위**: 사용자 제보 — 새로고침 제스처가 스크롤 기능과 겹쳐져서 잘 트리거되지 않는다.
...
- **다음 단계**: 재현 시 정확히 어느 화면/어떤 상황(목록 맨 위 vs 스크롤 중)에서 안 되는지부터 파악할 것. 근본 해결은 material3 1.3.0+에서 제공하는 비-실험적 `PullToRefreshBox` API로 이전하는 것인데, 이는 compose-bom/material3 버전을 올려야 해서 다른 화면(foundation/ui 등)까지 영향 범위가 넓어 신중한 검토가 필요 — 사용자와 먼저 상의할 것.
