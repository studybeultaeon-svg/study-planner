...
## Fixed (2026-09-19, 128차) — 모임 멤버 상세 캘린더의 일정 색이 실제 캘린더와 다름

- **설명**: 모임 → 멤버 상세 → 공부 → 캘린더에서 날짜를 눌러 나오는 일정 목록(데스크탑은 월 그리드 칩도)의 색이 그 멤버의 실제 캘린더 색과 달랐고, 4회독 이상 일정은 전부 회색으로 나왔다(사용자 보고).
- **원인**: 83차에 라이브 캘린더의 색상 원천이 `passIndex`/`passTotal` 그라데이션(`PassSchedule.passColor`)으로 바뀌었는데, 모임 동기화 payload(`ScheduleStat`)는 레거시 `color` 문자열만 실어 보내고 있어 멤버 상세 화면만 옛 팔레트에 남아 있었다. `color`는 passTotal==3 기본 케이스용 하위호환 라벨이라 4회독 이상은 `"pass{N}"`이 들어가고, 팔레트에 없는 값이라 `else` 분기(회색 `#AAAAAA`)로 떨어졌다.
- **상태**: Fixed
- **해결**: `ScheduleStat`에 `passIndex`/`passTotal`을 추가해 push/parse 양쪽에 싣고, 양 플랫폼 멤버 상세를 라이브 캘린더와 같은 `passColor`로 렌더링. 구버전이 올린 데이터는 `PassSchedule.legacyPassIndex(color)`로 추론([[DECISIONS.md]] 128차).
- **검증**: 양 플랫폼 릴리스 빌드 성공 + 호스트 배포. **실사용 미확인** — 멤버가 최신 빌드로 올린 일정이어야 색이 정확히 맞는다.

---

## Fixed (2026-09-18, 127차) — 126차 이후 데스크탑에서 폰 공부기록이 안 보임(126차 수정의 회귀)

- **설명**: 126차에서 공부기록 동기화를 고친 직후, **오히려 그전까지 잘 되던 데스크탑**에서 폰 공부기록이 하나도 안 보이게 됐다(사용자 보고).
- **원인**: 126차가 옛 칸(플랫폼 이름 그대로인 `"android"`/`"desktop"`)을 칸 이름만 보고 통째로 버리게 만들었다. "각 기기가 한 번 더 기록하면 새 칸으로 다시 올라가 저절로 복구된다"는 전제였으나 **기기 업데이트 시점이 다르면 성립하지 않는다** — 폰은 아직 126차 이전 빌드라 `"android"` 칸에 올리는데 먼저 업데이트된 데스크탑이 그 칸을 버렸다. 실측으로 그날 폰 기록 1236초가 데스크탑 기준 0초로 계산되는 것을 확인.
- **상태**: Fixed
- **해결**: 칸 이름 대신 **기록 지문(`시작시각|길이|이름`)** 으로 거른다 — 내 칸만 통째로 건너뛰고 나머지(옛 칸 포함)는 전부 읽되 로컬과 지문이 겹치는 것만 뺀다. 업데이트 순서와 무관하게 남의 기록은 항상 보이고 내 기록은 이중 계산되지 않는다([[DECISIONS.md]] 127차).
- **검증**: 데스크탑 JUnit `DeviceSyncMergeTest` 5/5(126차 규칙 사본에서 3/5 실패 역검증), 실제 Firebase 데이터에 두 규칙 대입 비교(0초 vs 1236초). 실기기 미확인.

---

## Fixed (2026-09-18, 127차) — 일일 사용한도가 안드로이드 기기 수만큼 느슨해짐

- **설명**: 폰과 태블릿이 같은 그룹을 쓰면 서로의 사용시간이 한도 판정에서 빠져, 일일 한도가 기기 수만큼 느슨해질 수 있었다(126차에 Open으로 남겨둔 항목).
- **원인**: `dailyUsage/{날짜}/{그룹}/{기기}`의 기기 값이 플랫폼 상수(`"android"`)라 폰·태블릿이 같은 칸을 덮어썼고(마지막 기기 값만 남음), 읽을 땐 그 칸을 자기 칸으로 보고 빼버렸다. 실측: 오늘 모든 그룹이 `android` 한 칸만 사용 중.
- **상태**: Fixed
- **해결**: 공부기록과 같은 `deviceSlotKey()`(`android-<id>`/`desktop-<id>`)로 올리고, 읽을 땐 내 칸만 빼고 나머지를 전부 더한다(옛 칸도 아직 업데이트 안 된 기기 몫일 수 있으므로 합산 — 한도는 덜 세는 쪽이 위험). 내 옛 칸은 앱 실행당 (날짜, 그룹) 한 번 첫 push 직전에 삭제([[DECISIONS.md]] 127차).
- **검증**: 데스크탑 JUnit(같은 플랫폼 기기 합산 + 옛 칸 합산) 통과, 안드로이드 컴파일/기존 테스트 통과. **실기기 확인은 아직** — 폰·태블릿 2대가 함께 있어야 검증된다.
